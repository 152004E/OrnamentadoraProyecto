...
import { CUHasLiked } from "../../application/UseCases/LikeUseCase/HasLiked";
...
export class LikesController {
  ...
  async contarLikesProyecto(req: Request, res: Response) {
    try {
      ...
      const usecase = new CUContarLikes(
        likesRepository,
        proyectoRepository,
        comentarioRepository
      );
  ...
  async hasLikedProyecto(req: Request, res: Response) {
    try {
      if (!req.user) {
  ...
  async hasLikedComentario(req: Request, res: Response) {
    try {
      if (!req.user) {
        return res.status(401).json({ message: "No autorizado" });
      }
